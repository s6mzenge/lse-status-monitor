...
def create_progression_graph(history, current_date, forecast=None, stream="all_other"):
    ...
    try:
        ...
        if len(rows) >= REGRESSION_MIN_POINTS:
            ...
            # NEU: Verwende das vollständige Blend-Modell,
            # aber nur **innerhalb** des Trainingssupports; außerhalb reines Theil–Sen.
            x_min = float(imodel.x_[0]) if len(imodel.x_) > 0 else -float("inf")
            x_max = float(imodel.x_[-1]) if len(imodel.x_) > 0 else float("inf")
            
            y_new = []
            for t in grid_ts:
                bd = _bizdays_float(first_ts, t)
                x_hours = bd * hours_per_day
                y_new.append(_support_aware_predict(imodel, x_hours))
            if len(y_new) > 0:
