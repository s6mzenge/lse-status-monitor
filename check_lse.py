...
def create_progression_graph(history, current_date, forecast=None, stream="all_other"):
    ...
    def _support_aware_predict(imodel, x_hours: float) -> float:
        """Stückweise Vorhersage: im Support LOESS-Blend, außerhalb Theil–Sen."""
        try:
            x_min = float(imodel.x_[0]); x_max = float(imodel.x_[-1])
        except Exception:
            x_min, x_max = -float("inf"), float("inf")
        if x_hours < x_min or x_hours > x_max:
            return float(imodel.ts_.predict(x_hours))
        return float(imodel._blend_predict_scalar(x_hours))
    ...
    nsteps = int(max(1, (right_bound - left_bound).total_seconds() // (3 * 3600)))
    grid_ts = [left_bound + timedelta(hours=3 * i) for i in range(nsteps + 1)]
    ...
    try:
        ...
        if len(rows) >= REGRESSION_MIN_POINTS:
            ...
            imodel = loess_frac_plot = 0.6
            tau_hours_plot  = 12.0
            try:
                if isinstance(forecast, dict) and 'params' in forecast:
                    pms = forecast['params']
                    loess_frac_plot = float(pms.get('eff_frac', pms.get('base_frac', loess_frac_plot)))
                    tau_hours_plot  = float(pms.get('eff_tau',  pms.get('base_tau',  tau_hours_plot)))
            except Exception:
                pass
            imodel = IntegratedRegressor(cal=cal, loess_frac=loess_frac_plot, tau_hours=tau_hours_plot).fit(rows)
            ...
            if len(y_new) > 0:
                ...
                # --- Nur für Darstellung glätten (Modell bleibt unverändert)
                PLOT_SMOOTH_NEU = True
                y_plot = y_new.copy()
                if PLOT_SMOOTH_NEU and len(y_plot) >= 7:
                    try:
                        from scipy.signal import savgol_filter
                        win = max(7, (len(y_plot)//25)*2 + 1)  # ungerade
                        if win >= len(y_plot):
                            win = len(y_plot) - (1 if len(y_plot)%2==0 else 2)
                        win = max(7, win)
                        y_plot = savgol_filter(y_plot, window_length=win, polyorder=3, mode="interp")
                    except Exception as e:
                        print(f"⚠️ Plot-Smoothing übersprungen: {e}")
                else:
                    y_plot = y_new
                ...
                ax.plot(grid_ts, y_plot, linewidth=2.6, 
                ...
                for name, ty in target_map.items():
                    ...
                    try:
                        t_star = None
                        # Bevorzuge ETA aus forecast, damit Stern und Text übereinstimmen
                        try:
                            if isinstance(forecast, dict):
                                t_star = forecast.get('eta_target_dt')
                                if t_star is not None:
                                    t_star = _to_naive_berlin(t_star)
                        except Exception:
                            t_star = None
                        if t_star is None:
                            ...
                            y_star = _support_aware_predict(imodel, x_hours_star)
    ...
    def _plot_eta(name, alt_pt, neu_pt):
        if alt_pt:
            ax.plot([alt_pt[0]], [alt_pt[1]], marker="*", markersize=12, zorder=10, linestyle="None",
